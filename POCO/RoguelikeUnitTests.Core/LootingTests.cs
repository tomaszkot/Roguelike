...
namespace RoguelikeUnitTests
{
  ...
  class LootingTests : TestBaseTyped<LootingTestsHelper>
  {
    ...
    public void KilledEnemyForScrolls()
    {
      ...
      {
        var env = CreateTestEnv(true, numEnemies: numEnemies);
        env.LootGenerator.Probability = new Roguelike.Probability.Looting();
      ...
      Assert.Less(scrolls.Count, numEnemies * mult);
    ...
    public void KilledEnemyForScrolls1()
    {
      ...
      //for (int run = 0; run < 5; run++)
      {
        ...
        {
          var numberOfNextTypeOfScrolls = nextScrollType.Count();
          Assert.Less(numberOfNextTypeOfScrolls, 40);
          int min = 1;
          int max = 34;
    ...
    {
      var env = CreateTestEnv(numEnemies: 60);
      env.LootGenerator.Probability = new Roguelike.Probability.Looting();
    ...
    [Test]
    public void AnimalsToLoot()
    {
      var env = CreateTestEnv();
      var anim = new Animal(env.GameManager.Container) { AnimalKind = AnimalKind.Hen };
      //env.GameManager.CurrentNode.SEt
      PlaceCloseToHero(anim);
      env.GameManager.AnimalsManager.AddEntity(anim);
      var li = new LootInfo(game, null);
      env.KillEnemy(anim);
      var lootItems = li.GetDiff();
      if (lootItems.Count == 0)
      ...
      Assert.GreaterOrEqual(lootItems.Count, 2);
    }
    ...
    public void Barrels2Loot()
    {
      ...
      var typesGrouped = scrolls.GroupBy(f => f.Kind).ToList();
      Assert.Greater(typesGrouped.Count, 6);
    ...
    public void PlainChests()
    {
      int mult = 1;
      var env = CreateTestEnv();
      int tilesToCreateCount = 150 * mult;
      int maxExpectedLootCount = 450;//chest gives 2-3 items
      ...
      Assert.Greater(lootInfo.newLoot.Count, 300);

      var fiks = lootInfo.Get<FightItemKind>();
      Assert.AreEqual(0, fiks.Where(i => i == FightItemKind.Smoke).Count());
      ...
      Assert.Greater(eqs.Count, 100);
      Assert.Less(eqs.Count, 160);
    ...
    public void FoodTests()
    {
      ...
      //TODO test names
      {
        var food = new Mushroom();
        food.MushroomKind = MushroomKind.BlueToadstool;
      ...
      {
        var food = new Mushroom();
        food.MushroomKind = MushroomKind.RedToadstool;
        Assert.AreEqual(food.Kind, FoodKind.Mushroom);
        Assert.AreEqual(food.StatKind, EntityStatKind.PoisonAttack);
      }
      {
        var food = new Mushroom();
        food.MushroomKind = MushroomKind.Boletus;
        Assert.AreEqual(food.Kind, FoodKind.Mushroom);
      ...
      {
        var food = new Food();
        food.SetKind(FoodKind.NiesiolowskiSoup);
        Assert.AreEqual(food.Name, "NiesiolowskiSoup");
        Assert.AreEqual(food.DisplayedName, "Niesiolowski Soup");
        Assert.AreEqual(food.tag1, "niesiolowski_soup");
    ...
    public void KilledEnemyGivesPotionFromTimeToTime()
    {
      for (int i = 0; i < loopCount; i++)
      {
        ...
        var li = new LootInfo(game, null);
        var ens = env.KillAllEnemies();
        var np = ens.Where(i => i.PowerKind != EnemyPowerKind.Plain);
        if (np.Count() <= 1)
          np.First().SetNonPlain(false);
        np = ens.Where(i => i.PowerKind != EnemyPowerKind.Plain);
        var lootItems = li.GetDiff();
    ...
    public void RecipeVariety()
    {
      ...
      for (int i = 0; i < 10; i++)
      {
        var lootReturned = game.GameManager.LootManager.EnsureVariety(new Chest(Container), LootSourceKind.PlainChest, recipe);

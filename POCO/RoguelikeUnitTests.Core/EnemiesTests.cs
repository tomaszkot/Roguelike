...
namespace RoguelikeUnitTests
{
  ...
  class EnemiesTests : TestBase
  {
    ...
    public void TestPowerIncrease()
    {
      ...
      for (var levelIndex = 0; levelIndex < GenerationInfo.DefaultMaxLevelIndex; levelIndex++)
      {
    ...
    public void TestSpeedInWater()
    {
      ...
      //TODO why nulls?
      for (int col = 0; col < game.Level.Width; col++)
      {
        for (int row = 0; row < game.Level.Height; row++)
        {
          if (game.Level.Tiles[row, col] == null)
          {
            game.Level.SetTile(new Tile(), new System.Drawing.Point(col, row));
          }
        }
      }
      ...
      Assert.True(game.Level.GetSurfaceKindsUnderTile(enemy).Contains(SurfaceKind.ShallowWater));
      ...
      if (enemyDistFromHeroNew != enemyDistFromHero - 2)
      {
        ...
        enNeibs.ForEach(i =>
        {
          Debug.Write("tile neib to hero: "+i + ", Surface: "+ game.Level.GetSurfaceKindsUnderTile(i).FirstOrDefault() + "\r\n");
        }
        );
      }

    ...
    {
      {
        ...
        Assert.AreEqual(hp.Name, "Health Potion");
        Assert.AreEqual(hp.DisplayedName, hp.Name);
        Assert.Greater(hp.PrimaryStatDescription.Length, 5);
      }
      {
        var mana = new Potion();
        mana.SetKind(Roguelike.Tiles.Looting.PotionKind.Mana);
        Assert.AreEqual(mana.Name, "Mana Potion");
        Assert.AreEqual(mana.DisplayedName, mana.Name);
      ...
      }
      {
        ...
        Assert.AreEqual(poison.Name, "Antidote Potion");
        Assert.AreEqual(poison.Name, poison.DisplayedName);

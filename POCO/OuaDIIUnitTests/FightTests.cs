...
namespace OuaDIIUnitTests
{
  ...
  class FightTests : TestBase
  {
    ...
    public void TestReqStatsAxes()
    {
      ...
      var names = new[] { "sickle", "hand_axe", "solid_axe", "axe", "double_axe", "war_axe" };
    ...
    {
      var game = CreateWorld();
      var names = new[] { "war_dagger", "solid_hammer", "war_sword", "war_axe" };
      ...
      Assert.Greater(ratio, 1.4f);
      Assert.Less(ratio, 2f);
      Assert.AreEqual(maxMelee.Key, "solid_hammer");
    ...
    public void TestAttackFireBall()
    {
      ...
      Assert.AreEqual(ad4.CurrentTotal, baseElementalDamage);
    }
    ...
    [Test]
    public void EnemyWoundedLowerDefence()
    {
      var world = CreateWorld();
      var enemies = world.GetTiles<OuaDII.Tiles.LivingEntities.Enemy>();
      var notWounded = enemies.Where(i => !i.IsWounded).First();
      var oldDefence = notWounded.Stats.GetCurrentValue(EntityStatKind.Defense);
      var oldHealth = notWounded.Stats.Health;

      notWounded.SetIsWounded(true);

      Assert.Less(notWounded.Stats.GetCurrentValue(EntityStatKind.Defense), oldDefence);
    ...
    [Repeat(2)]
    public void ArrowFightItemTest(string eqName1, string eqName2)
    {
      var world = CreateWorld();
      var hero = GameManager.Hero;

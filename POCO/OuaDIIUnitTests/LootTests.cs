...
namespace OuaDIIUnitTests
{
  ...
  class LootTests : TestBase
  {
    ...
    {
      var infoW = new OuaDII.Generators.GenerationInfo();
    ...
    [Test]
    public void EquipmentMaterialFromLootManager()
    {
      var world = CreateWorld();
      GameManager.LootGenerator.Probability.SetLootingChance(LootSourceKind.Enemy, LootKind.Equipment, 1);
      ...
      for (int enLevel = 1; enLevel <= 7; enLevel++)
      {
        ...
        for (int i = 0; i < 100; i++)
        {
          ...
          eqs.AddRange(loot.Where(k => k is Equipment eq && eq.IsMaterialAware()).Cast<Equipment>().ToList());
        }
      ...
      foreach (var levelToEq_ in levelToEqCount)
      {
        ...
        if (levelToEq_.Key <= MaterialProps.IronDropLootSrcLevel)
          Assert.True(eqs.All(i => i.Material == Roguelike.Tiles.EquipmentMaterial.Bronze));
        else
        {
          ...
          {
            Assert.True(eqs.Any(i => i.Material == Roguelike.Tiles.EquipmentMaterial.Bronze));
            Assert.True(eqs.Any(i => i.Material == Roguelike.Tiles.EquipmentMaterial.Iron));
          }
          else
          {
            Assert.True(eqs.Any(i => i.Material == Roguelike.Tiles.EquipmentMaterial.Iron));
            Assert.True(eqs.Any(i => i.Material == Roguelike.Tiles.EquipmentMaterial.Steel));
          }
        }
      }
    }
    ...
    public void EquipmentMaterial()
    {
      ...
      foreach (var wpn in wpns)
      {
        ...
        if (wpn.Source.Level <= MaterialProps.IronDropLootSrcLevel)
          Assert.True(wpn.Material == Roguelike.Tiles.EquipmentMaterial.Unset ||
                    wpn.Material == Roguelike.Tiles.EquipmentMaterial.Bronze);
        else if (wpn.Source.Level <= MaterialProps.SteelDropLootSrcLevel)
        {
          var ok = wpn.Material == Roguelike.Tiles.EquipmentMaterial.Unset ||
                    wpn.Material == Roguelike.Tiles.EquipmentMaterial.Iron;
          if (!ok)
          {
            mismatched++;
        ...
        { //RandHelper.GetRandomDouble() > 0.5f --> Steel
          Assert.True(wpn.Material == Roguelike.Tiles.EquipmentMaterial.Unset ||
                    wpn.Material == Roguelike.Tiles.EquipmentMaterial.Iron ||
                    wpn.Material == Roguelike.Tiles.EquipmentMaterial.Steel
                    );
        }
      }
    ...
    public void EatableConsumption()
    {
      ...
      int effCounter = 0;
      GameManager.EventsManager.ActionAppended += (object s, GameEvent ga) =>
      {
    ...
    public void KilledEnemyGivesEveryLoot()
    {
      {
        var infoW = new OuaDII.Generators.GenerationInfo();
        infoW.Counts.WorldEnemiesCount = 200;
        ...
        foreach (var kind in allKinds)
        {
          if (kind == LootKind.Book)
            continue;
          var count = newLoot.Where(i => i.LootKind == kind).Count();
          Assert.Greater(count, 0);
          int maxCount = 30;
          if (kind == LootKind.Potion)
            maxCount = 40;
          else if (kind == LootKind.Equipment)
            maxCount = 70;//Chemps and Bosses throw eq
    ...
    public void LootFromChampion()
    {
      //for (int t = 0; t < 3; t++)
      {
        ...
        GameManager.Hero.Level = 2;
        var eqsBefore = world.GetTiles<Loot>();
        AllEnemies.Cast<Enemy>().ToList().ForEach(i => i.SetNonPlain(false));
        AllEnemies.Cast<Enemy>().ToList().ForEach(i => i.SetLevel(2));
        KillAllEnemies();
        var newLoot = GetDiff(eqsBefore);
        Assert.Greater(newLoot.Count, 0);

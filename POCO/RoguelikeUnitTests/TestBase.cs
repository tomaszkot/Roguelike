...
namespace RoguelikeUnitTests
{
  ...
  public class TestBase
  {
    ...
    protected void PlaceCloseToHero(IDestroyable destr)
    {
      var hero = game.Hero;
      var empty = GameManager.CurrentNode.GetClosestEmpty(hero);
      GameManager.CurrentNode.SetTile(destr as Tile, empty.point);
      if (destr is LivingEntity le && !GameManager.EnemiesManager.AllEntities.Contains(le))
        GameManager.EnemiesManager.AddEntity(le);
    }

...
namespace OuaDIIUnitTests
{
  ...
  class EquipmentTests : TestBase
  {
    ...
    public void HoundEquipmentTest()
    {
      ...
      Assert.Greater(arm2.Defense, arm1.Defense);
    }
    ...
    public void EquipmentMaterialTest()
    {
      ...
      var ironDamage = 0;
      {
        ...
        sword.SetMaterial(EquipmentMaterial.Iron);
      ...
      }
      {
        ...
        sword1.SetMaterial(EquipmentMaterial.Steel);
...

      {
        ...
        axe.SetMaterial(EquipmentMaterial.Iron);
...

      {
        ...
        var baseVal = MaterialProps.BronzeDaggerBaseDamage + 1;//lvl 4 make it +1
        Assert.AreEqual(sword1.Damage, baseVal);

        sword1.SetMaterial(EquipmentMaterial.Steel);
        Assert.Greater(sword1.Damage, MaterialProps.BronzeDaggerBaseDamage);
        Assert.LessOrEqual(sword1.Damage, baseVal * 3);
      }
    ...
    public void ArmorRequirementsTest()
    {
      ...
      Assert.Greater(cape.Defense, ragged_tunic.Defense);
      ...
      Assert.Greater(padded_armor.Defense, cape.Defense);
      ...
      Assert.Greater(solid_leather_armor.Defense, padded_armor.Defense);
      ...
      Assert.Greater(full_plate_armor.Defense, padded_armor.Defense);
    ...
    public void ShieldRequirementsTest()
    {
      ...
      Assert.Greater(buckler.Defense, damaged_buckler.Defense);
      ...
      Assert.Greater(long_shield.Defense, buckler.Defense);
      ...
      Assert.Greater(kings_bucket.Defense, long_shield.Defense);
    ...
    public void GlovesRequirementsTest()
    {
      ...
      Assert.Greater(leather_gloves.Defense, worn_gloves.Defense);
      ...
      Assert.Greater(gauntlets.Defense, leather_gloves.Defense);
    ...
    public void HelmetsRequirementsTest()
    {
      ...
      Assert.Greater(helm.Defense, cap.Defense);
      ...
      Assert.Greater(holly_helm.Defense, helm.Defense);
    ...
    [Test]
    public void TorchFightItemReplaceShieldTest()
    {
    ...
    [Test]
    public void StackedCountForHotBarTest()
    {
      var world = CreateWorld();
      var hero = GameManager.OuadHero;
      ...
      //prevent from auto put on torch by put on shield
      var lg = GameManager.LootGenerator;
      Assert.Null(hero.GetFightItemFromCurrentEq());
      var fi = CollectTorches(hero, 10);
      Assert.NotNull(hero.GetFightItemFromCurrentEq());
      Assert.AreEqual(hero.GetStackedCountForHotBar(fi), 10);
            
      var fi1 = new ProjectileFightItem(FightItemKind.ThrowingTorch, hero) { Count = 1 };
      hero.Inventory.Add(fi1);
      Assert.AreEqual(hero.GetStackedCountForHotBar(fi), 11);

      var fi2 = new ProjectileFightItem(FightItemKind.ThrowingTorch, hero) { Count = 1 }; 
      hero.CurrentEquipment.Add(fi2);
      Assert.AreEqual(hero.GetStackedCountForHotBar(fi), 12);

      var fi4 = new ProjectileFightItem(FightItemKind.ThrowingTorch, hero) { Count = 1 };
      hero.CurrentEquipment.Remove(fi4, new RemoveItemArg() { StackedCount = 1 });
      Assert.AreEqual(hero.GetStackedCountForHotBar(fi), 11);

      hero.MoveEquipmentCurrent2Inv(fi, CurrentEquipmentKind.Shield);

      Assert.Null(hero.GetFightItemFromCurrentEq());
      Assert.AreEqual(hero.GetStackedCountForHotBar(fi), 11);
      Assert.AreEqual(hero.Inventory.GetStackedCount(fi), 11);


      var fi3 = new ProjectileFightItem(FightItemKind.ThrowingTorch, hero) { Count = 1 }; 
      hero.Inventory.Remove(fi3);
      Assert.AreEqual(hero.GetStackedCountForHotBar(fi), 10);
    ...
    public void TorchFightItemCountTest()
    {
      ...
      var lg = GameManager.LootGenerator;
      var fi = CollectTorches(hero, 10);
      Assert.False(hero.Inventory.Contains(fi));//were moved to hand
      Assert.True(hero.ShortcutsBar.HasItem(fi));
      ...
      var fiAtBar = hero.ShortcutsBar.GetStackedAt(digit);
      Assert.AreEqual(fiAtBar.Count, 10);

      CollectTorches(hero, 1);
      
      fiAtBar = hero.ShortcutsBar.GetStackedAt(digit);
      Assert.AreEqual(fiAtBar.Count, 11);
      GameManager.Save(false);
      GameManager.Load(GameManager.Hero.Name, false);

      fiAtBar = GameManager.OuadHero.ShortcutsBar.GetStackedAt(digit);
      Assert.AreEqual(fiAtBar.Count, 11);
    }

...
namespace OuaDIIUnitTests
{
  ...
  class QuestTests : TestBase
  {
    ...
    private Equipment TestSmithQuest(QuestKind questKind, MinedLootKind lootKind, ref OuaDII.Discussions.DiscussPanel discussPanel)
    {
      ...
      //topic = {Come on, it's just one sword...->There is a way this can be done. If you deliver me 10 pieces of the iron ore I can devote part of it to making you a weapon. Unset}...
    ...
    )
    {
      ...
      var topic = discussPanel.BoundTopics.TypedItems[0].Item.AsOuaDItem();//Where would I find...
      var ok = discussPanel.ChooseDiscussionTopic(topic);
      Assert.True(ok);

      if (lootKind == MinedLootKind.IronOre)
      {
      ...
      var sword = reward as Roguelike.Tiles.Looting.Weapon;
    ...
    public void HourglassForMiller()
    {
      ...
      var panel = AssignHourglassQuest();

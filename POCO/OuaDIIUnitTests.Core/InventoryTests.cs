...
namespace OuaDIIUnitTests
{
  ...
  {
    string portalScrollAssetTag = "portal_scroll";
    ...
    public void SaveLoadTest()
    {
      ...
      var arm = new Roguelike.Tiles.Looting.MagicDust();
    ...
    public void TradeWithMerchantPossibleIfCapacityReachedButStackableAlreadyInBasket()
    {
      ...
      var portalScroll = GameManager.LootGenerator.GetLootByAsset(portalScrollAssetTag) as Scroll;
      Assert.True(portalScroll.StackedInInventory);
      portalScroll.Count = 1;
      Assert.True(hero.Inventory.Add(portalScroll));

      var lootExtra = GameManager.LootGenerator.GetLootByAsset("identify_scroll") as StackedLoot;
      //out of space, because Capacity == 1 
      Assert.False(hero.Inventory.Add(lootExtra));

      Assert.AreEqual(hero.Inventory.GetStackedCount(portalScroll as StackedLoot), 1);

      //var initPortalScrolls = merchant.Inventory.GetStackedCount(portalScrollAsset);
      var initPortalScrolls = merchant.Inventory.GetStackedCount(portalScroll);
      lootExtra = GameManager.LootGenerator.GetLootByAsset(portalScrollAssetTag) as StackedLoot;
      lootExtra.Count = 5;
      hero.Gold = 1000;
      Assert.True(merchant.Inventory.Add(lootExtra));
      var sc = merchant.Inventory.GetStackedCount(lootExtra);
      if (sc != 5)
      {
        int k = 0;
        k++;
      }
      Assert.AreEqual(sc, 5+ initPortalScrolls);

      //sell to hero
      var sellItemArg = new RemoveItemArg();
      sellItemArg.StackedCount = sc;
      ...
      Assert.AreEqual(hero.Inventory.GetStackedCount(portalScroll), 1 + sc);
    }
    ...
    public void TradeWithMerchantCheckPricesForStacked()
    {
      ...
      var lootExtra = GameManager.LootGenerator.GetLootByAsset(portalScrollAssetTag) as StackedLoot;
      lootExtra.Count = 20;
      Assert.True(merchant.Inventory.Add(lootExtra));
    ...
    public void HeroChest()
    {
      ...
      Assert.AreEqual(heroGold, hero.Gold);
      Assert.AreEqual(hero.Chest.Inventory.ItemsCount, 1);
      hero.Chest.Inventory.Add(new Roguelike.Tiles.Looting.MagicDust());

      GameManager.Save(false);
      GameManager.Load(hero.Name, false);
      hero = GameManager.OuadHero;// as OuaDII.Tiles.LivingEntities.Hero;
      Assert.AreEqual(hero.Chest.Inventory.ItemsCount, 2);
    }
    ...
    public void MoveEquipment2AllySaveMoveEqAgain()
    {
      ...
      SaveLoad();
      var heroLoaded = GameManager.OuadHero;
      ...
      var axe = GameManager.LootGenerator.GetLootByTileName<Weapon>("axe");
      Assert.True(heroLoaded.Inventory.Add(axe));
      Assert.NotNull(GameManager.SellItem(axe, heroLoaded, heroLoaded.Inventory, allyLoaded, allyLoaded.Inventory));

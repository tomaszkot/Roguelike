...
namespace Entities
{
  public partial class EquipmentItem : Entity
  {
	...
	private string defaultConsumableTexture = "res://Sprites/Loot/Consumable/apple.png";
	private string HealthPotionTexture = "res://Sprites/Loot/Consumable/health_potion.png";
	private string ManaPotionTexture = "res://Sprites/Loot/Consumable/mana_potion.png";
  ...
  { EquipmentKind.Helmet, "Helmets/" }, { EquipmentKind.Shield, "Shields/" } };
	...
	public void SetSprite()
	{
	  ...
	  else if (loot is Consumable c)
	  {
		string texture = pathToConsumables + loot.tag1 + ".png";
		if (c is Potion p)
		{
		  if (p.IsPotionKind(PotionKind.Health))
		  {
			LoadTexture(HealthPotionTexture);
		  }
		  else if (p.IsPotionKind(PotionKind.Mana))
		  {
			LoadTexture(ManaPotionTexture);
		  }
		}
		if (ResourceLoader.Exists(texture))
		  LoadTexture(texture);

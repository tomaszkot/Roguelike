  ...
  public class LootManager
  {
    ...
    public List<Loot> TryAddForLootSource(ILootSource lootSource)//Barrel, Chest from attackPolicy.Victim
    {
      ...
      try
      {
        ...
        //  Name = "Rusty sword",
        //  Kind = Weapon.WeaponKind.Sword
        //};
        //wpn.SetLevelIndex(1);
        //wpn.SetMaterial(EquipmentMaterial.Steel);
        //lootSource.ForcedReward = wpn;


        if (lootSource.ForcedReward != null)
        {
    ...
    private List<Loot> TryAddForNonEnemy(ILootSource lootSource)
    {
      ...
      if (lootSource is Animal anim)
      {
        if (anim.tag1 != "boar_butcher")
        {
          var loot1 = new Food(FoodKind.Meat);
          if (GameManager.AddLootReward(loot1, lootSource, true))//add loot at closest empty
            lootItems.Add(loot1);

          if(anim.AnimalKind == AnimalKind.Pig || anim.AnimalKind == AnimalKind.Deer)
              loot1.Count = 2;
        }

        if (anim.AnimalKind == AnimalKind.Hen || anim.AnimalKind == AnimalKind.Rooster)
        {
          var fe = new Feather();
          if (GameManager.AddLootReward(fe, lootSource, true))//add loot at closest empty
            lootItems.Add(fe);
        }
        return lootItems;
      }
      ...
      if (lootSource is Barrel bar && bar.BarrelKind == BarrelKind.OilBarrel)
      {
        var et = new Tile();
        GameManager.ReplaceTile(et, lootSource as Barrel);
        GameManager.SpreadOil(et);
      ...
      var enemyFromChest = lootSource is Chest ch
        && ch.ChestVisualKind == ChestVisualKind.Grave
        && RandHelper.GetRandomDouble() < GenerationInfo.ChanceToGenerateEnemyFromGrave;

      if (enemyFromChest ||
          (lootSource is Barrel barrel && !(lootSource is DeadBody) && RandHelper.GetRandomDouble() < GenerationInfo.ChanceToGenerateEnemyFromBarrel)
          )
      {
        if (enemyFromChest)
      ...
      {
        bool repl = GameManager.ReplaceTile(loot, lootSource as Barrel);
    ...
    public Loot EnsureVariety(ILootSource lootSource, LootSourceKind lsk, Loot loot)
    {
      if (loot is Recipe rec)
      {
        ...
        {
          var newLoot = GameManager.LootManager.LootGenerator.LootFactory.MiscLootFactory.GetRandomRecipe(lootSource.Level);
          if(newLoot.Name == loot.Name)
            newLoot = GameManager.LootManager.LootGenerator.LootFactory.MiscLootFactory.GetRandomRecipe(lootSource.Level);
    ...
    Loot TryAddLootForDeadEnemy(Enemy enemy)
    {
      ...
      if (addRichConsumableOrOtherReward)
      {
        ...
        if (RandHelper.GetRandomDouble() > 0.5f)
        {
          ...
          var exLoot = GameManager.LootGenerator.GetRandomLoot(enemy.Level, LootKind.Scroll);
    ...
    protected virtual List<Loot> GetExtraLoot(ILootSource lootSource, Loot primaryLoot)
    {
      ...
      {
        var fis = Weapon.AllBowLikeAmmoKinds.ToList();
        fis.Add(FightItemKind.ThrowingKnife);
